...
| AI エージェント(イベント編集) | `src/server/application/useCases/parseEventEditText.ts`・`gateways/claudeEventEditGateway.ts` | プロンプトの正本は [docs/SYSTEM_PROMPT.md](../SYSTEM_PROMPT.md) §3(`gateways/eventEditPrompt.ts` と同期必須)、判断は [ADR 0013](../adr/0013NaturalLanguageEventEdit.md)。フォールバックを持たない(Claude 専用) |
...
| イベント作成・編集・回答(永続化) | `src/server/application/useCases/events.ts`・`src/server/repositories/eventRepository.ts`・`src/server/infrastructure/repositories/sqliteEventRepository.ts` | DB スキーマ変更は DDL と `tests/store.test.ts` をセットで。レイヤの判断は [ADR 0001](../adr/0001HarnessAndArchitecture.md)、編集のアクセス制御・候補の差分適用は [ADR 0011](../adr/0011EventEditing.md)、回答の修正(同名上書き)は [ADR 0012](../adr/0012ResponseCorrection.md)。マイグレーション機構は未導入(導入するなら ADR 起票) |
...
| ログ / 監査 / 障害調査 | `src/server/infrastructure/logging/` | イベント一覧・調査手順は [runbook/observability.md](../runbook/observability.md)、判断は [ADR 0008](../adr/0008StructuredLoggingAndAudit.md)。新規 API ルートは必ず `withApiLogging` で包む。PII はログに残さない(ユーザー同意時のみ例外、[ADR 0010](../adr/0010OptInDiagnosticLogging.md)) |
