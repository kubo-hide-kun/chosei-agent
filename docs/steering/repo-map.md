...
| ログ / 監査 / 障害調査 | `src/server/infrastructure/logging/` | イベント一覧・調査手順は [runbook/observability.md](../runbook/observability.md)、判断は [ADR 0008](../adr/0008StructuredLoggingAndAudit.md)。新規 API ルートは必ず `withApiLogging` で包む。PII はログに残さない |
