...
| AI エージェント(自然文解析) | `src/server/application/useCases/parseScheduleText.ts`・`src/server/infrastructure/gateways/`・`src/server/domain/scheduleText.ts` | プロンプトの正本は [docs/SYSTEM_PROMPT.md](../SYSTEM_PROMPT.md)、解析戦略は [ADR 0003](../adr/0003AgentParsingStrategy.md)(`gateways/schedulePrompt.ts` と同期必須)。フォールバック変更時は `tests/fallback.test.ts` を更新 |
